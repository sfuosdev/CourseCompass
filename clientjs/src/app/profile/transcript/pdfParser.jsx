...
async function parsePDFToCourses(pdfData) {
  try {
    ...
    let currentYear = '';
    let currentTerm = '';
    let hasUnofficialTranscript = false;
    ...
    for (let i = 1; i <= pdf.numPages; i++) {
      ...
      for (const item of pageText.items) {
        ...
        if (text.toLowerCase().includes('unofficial transcript')) {
          hasUnofficialTranscript = true;
        }
        ...
        // match courses
        const courseMatch = text.match(/\b(?:[A-Z]{2,4}\s?\t?\d{3}[A-Z]?|FAL|FAN)\b/g);
        if (courseMatch && currentYear && currentTerm) {
          const modifiedCourses = courseMatch.map(course => {
            return (course === 'FAL' || course === 'FAN') ? course + ' X99' : course;
          });
          coursesByYearAndTerm[`${currentYear} ${currentTerm}`].push(...modifiedCourses);
        }
      }
    }
