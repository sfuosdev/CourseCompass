const TitleLabel = ({ sectionSchedule, idx}) => {
  let title;
  if(idx === 0) {
    title = <div className="inline-block font-semibold text-[15px] h-[20%] mb-[5px]">
      Lecture:
    </div>
  }
  else {
    if(sectionSchedule[0].sectionCode === "LAB") {
      title = <div className="inline-block font-semibold text-[15px] h-[25%] mb-[5px]">
        Labs:
      </div>
    }
...
const ClassSchedule = ({ offering }) => {
  const schedule = offering.courseSchedule.map((sectionSchedule, idx) => (
    ...
    >
      {idx <= 1? <TitleLabel sectionSchedule={sectionSchedule} idx={idx} /> : ""}
