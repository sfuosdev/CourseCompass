...
const SearchBar = ({ page, onSearchChange }) => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (searchTerm.trim() !== "") {
          const response = await fetch(
            `/api/search/searchCourses?searchTerm=${searchTerm}`,
            { signal }
          );
    ...
    if (searchTerm.trim() !== "") fetchSearchResults();
  ...
  switch (page) {
    case "LandPage":
      return (
        <div className="flex inline-flex w-[80vw] md:w-[50vw] mt-[2rem] md:mt-[4rem] rounded-md border-opacity-50 justify-center">
          <svg
          ...
          >
            <g clip-path="url(#clip0_13_778)">
              ...
              />
              <path
                d="M15.8032 15.8037L20.9998 21.0003"
                stroke="#23272A"
                stroke-opacity="0.6"
                stroke-width="1.5"
                stroke-linecap="round"
    ...
    case "other":
      return (
        <div className="flex flex-col items-center mt-4">
          <div
            className="flex inline-flex w-[80vw] md:w-[50vw] rounded-md border-opacity-50 justify-center"
                ...
                {showSearchResults && (
                  <ul className="">
                    {searchResults.map((course) => (
                      <li
                        key={course._id}
                        className="hover:bg-primary-yellow hover:text-black"
                      >
    ...
    case "other":
      return (
        <div className="flex flex-col items-center mt-4">
          <div
            className="flex inline-flex h-[2.5rem] rounded-full"
            ref={searchContainerRef}
          >
            <input
              type="text"
