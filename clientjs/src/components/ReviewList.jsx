...
export default function Review({ review, user }) {
  ...
  function handleUpVote() {
    ...
    } else setlikesCounter(likeCounter - 1);
    // fix counter later
  ...
  function handleDownVote() {
    ...
    } else setdislikesCounter(dislikeCounter - 1);
  }
  ...
  const view =
    user === `` ? (
      <>
        ...
        >
          {`${upArrows[0]}`}{" "}
          <span className="text-[8px] md:text-[12px]">{likeCounter}</span>
        </Link>
        <Link
          href={`http://localhost:3000`}
          className="px-2 text-[12px] md:text-[20px]"
        >
          {`${downArrows[0]}`}{" "}
          <span className="text-[8px] md:text-[12px]">{dislikeCounter}</span>
        </Link>
      </>
    ) : (
      <>
        ...
        >
          {`${like == true ? upArrows[1] : upArrows[0]}`}{" "}
          <span className="text-[8px] md:text-[12px]">{likeCounter}</span>
        </button>
        ...
        >
          {dislike == true ? downArrows[1] : downArrows[0]}{" "}
          <span className="text-[8px] md:text-[12px]">{dislikeCounter}</span>
        </button>
      </>
    );
  ...
  return (
    <div className="relative md:p-4 md:w-3/4 flex flex-row border-b-2 border-black hover:bg-gray">
      ...
      <div className="p-3 md:p-0 flex flex-col w-auto justify-between">
        ...
        <div className="grid grid-cols-3 text-[10px] md:text-[18px]">
          ...
          <div className="font-semibold">
            Clarity:{" "}
            <span className="font-normal">{`${review.clarity}/5`}</span>
          </div>
          <div className="font-semibold">
            Engagement:{" "}
            <span className="font-normal">{`${review.engagement}/5`}</span>
          </div>
        </div>
      </div>
    </div>
